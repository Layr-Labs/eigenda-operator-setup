...
if ! [ -f ./g1.point ]; then
  ...
  wget https://srs-mainnet.s3.amazonaws.com/kzg/g1.point --output-document=./g1.point
  DOWNLOADED_FILE=true
else
  echo "g1.point already exists."
fi

if ! [ -f ./g2.point.powerOf2 ]; then
  echo "g2.point.powerOf2 does not exist."
  echo "Downloading g2 point powerOf2. This will take few seconds"
  wget https://srs-mainnet.s3.amazonaws.com/kzg/g2.point.powerOf2 --output-document=./g2.point.powerOf2
  DOWNLOADED_FILE=true
fi
...
if [ "$DOWNLOADED_FILE" = true ]; then
  ...
  if (sha256sum -c srssha256sums.txt); then
