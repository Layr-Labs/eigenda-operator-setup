...
Alongside the aforementioned dashboards, EigenDA provides baseline alerting over key metrics such as the number of chunks signed over a period of time. These alerts can be accessed [here](monitoring/alerting). Again, they should be imported automatically with the rest of the setup. To configure notifications, refer to the [monitoring guide](monitoring/README.md).
